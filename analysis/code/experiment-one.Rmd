...
```{r plot.diff.examples, echo = F, warning = F, message = F}
...
```{r bae.examples, message = F, echo = F, warning = F}
...
```{r sample.gae, message = F, echo = F}
gae()
```

## function `gavglme` and `lmer` models

```{r lme.function, message = F}
...
```{r sample.model.summary, message = F}
summary(gavglme(150, 200, "frontocentral", "frontal"))
```

```{r sample.models, eval = F, message = F}
...
x <- int_compare_scalps(75, 125)
y <- int_compare_scalps(225, 275)
```
