...
```{r}
gavglme <- function(TP1, TP2, where = "", REML = TRUE) {
  ...
  if (where != "") {
    ...
    n <- summary(lmer(formula = avg.mean ~ region * condition * side + (1 + condition | subject), 
                    data = lme_data,
                    REML = REML)) }
